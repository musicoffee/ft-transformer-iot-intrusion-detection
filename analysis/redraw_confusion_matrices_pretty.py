...
# MDPI proof requirement:
# Use mathematical minus sign “−” (U+2212), not hyphen-minus “-”, for negative values.
plt.rcParams["axes.unicode_minus"] = True


def format_number_with_math_minus(value: float, precision: int = 3) -> str:
    """
    Format numerical feature values for figure labels.
    Negative values use the mathematical minus sign “−” required by MDPI.
    """
...
def make_local_bar_plot(
    ...
    labels = [
        f"{name} = {format_number_with_math_minus(feat_values[idx], precision=4)}"
...
def make_combined_plot(
    ...
    def prepare(shap_row, x_row):
        ...
        feat_values = x_row[plot_indices]
        labels = [
            f"{n}={format_number_with_math_minus(v, precision=3)}"
            for n, v in zip(names, feat_values)
        ]
        colors = ["#d62728" if v >= 0 else "#1f77b4" for v in values]
